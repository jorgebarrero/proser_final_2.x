...
export async function dashboardOutboundIndicators(userSelection) {
  ...
  let dashboardOutboundCurrentCallsInicators = await dashboardOutboundCurrentCallsInicatorsFunction(
    userSelection
  );
  ...
  let colors = [
    {
      ...
      callsOnQueue: onColorForCallsOnQueue(
        dashboardOutboundCurrentCallsInicators[0].maxWaitTimeOnQue,
...
/**************************************** */
// current calls
async function dashboardOutboundCurrentCallsInicatorsFunction(userSelection) {
  let result = null;
  let resume_error = false;
  let query = `
