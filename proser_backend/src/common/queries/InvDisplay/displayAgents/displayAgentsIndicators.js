...
export async function displayAgentsIndicators(userSelection) {
  ...
  let displayAgentsCurrentCallsInicators = await displayAgentsCurrentCallsInicatorsFunction(
    userSelection
  );
    ...
    colors = [
      {
        ...
        AgentsAtentionLevel: onColorForPercentage(
          displayAgentsCallsIndicators[0].AgentsAtentionLevel,
          scale[0]
        ),
        ...
        callsOnQueue: onColorForCallsOnQueue(
          displayAgentsCurrentCallsInicators[0].maxWaitTimeOnQue,
...
/**************************************** */
// current calls
async function displayAgentsCurrentCallsInicatorsFunction(userSelection) {
  let result = null;
  let resume_error = false;
  let query = `
-- displayAgentsCurrentCallsInicatorsFunction --------------
-- FIELDS
SELECT
