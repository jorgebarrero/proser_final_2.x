...
export class CrudInvSupervisorDetailComponent implements OnInit {
  ...
  onFillForm() {
    ...
    this.registerForm = this.formBuilder.group({
      ...
      inv_supervisor_type_json: [
        this.selection.inv_supervisor_people_json.type
      ],
      inv_supervisor_schedule_json: [
        this.selection.inv_supervisor_time_json.schedule
      ],
      inv_supervisor_client_json: [
        this.selection.inv_supervisor_operation_json.client
      ],
      inv_supervisor_queue_json: [
        this.selection.inv_supervisor_operation_json.queue
      ],
      inv_supervisor_service_json: [
        this.selection.inv_supervisor_operation_json.service
      ],
      inv_supervisor_campaign_json: [
        this.selection.inv_supervisor_operation_json.campaign
      ]
    });
  }
  ...
  // CREATE RECORD
  createRecord(temp) {
    // CREATE NEW JSON FIELDS
    temp.inv_supervisor_people_json = new PeopleJsonModel();
    temp.inv_supervisor_operation_json = new OperationJsonModel();
    temp.inv_supervisor_time_json = new TimeJsonModel();

    // ASIGN FORM VALUES
    temp.inv_supervisor_people_json.type = temp.inv_supervisor_type_json;
    temp.inv_supervisor_time_json.schedule = temp.inv_supervisor_schedule_json;
    temp.inv_supervisor_operation_json.client = temp.inv_supervisor_client_json;
    temp.inv_supervisor_operation_json.queue = temp.inv_supervisor_queue_json;
    temp.inv_supervisor_operation_json.service =
      temp.inv_supervisor_service_json;
    temp.inv_supervisor_operation_json.campaign =
      temp.inv_supervisor_campaign_json;

    let query = temp;

    // STRINGIFY JSON FIELDS
    query.inv_supervisor_people_json = JSON.stringify(
      temp.inv_supervisor_people_json
    );
    query.inv_supervisor_operation_json = JSON.stringify(
      temp.inv_supervisor_operation_json
    );

    query.inv_supervisor_time_json = JSON.stringify(
      temp.inv_supervisor_time_json
    );
    ...
    this.invSupervisorService.postRecord(query).subscribe(
      data => {
        alert(
          `Registro agregado satisfactoriamente, ${data.inv_supervisor_id}, ${
            data.inv_supervisor_name
          }`
        );
  ...
  // EDIT RECORD
  editRecord(temp): InvSupervisorModel {
    // CREATE NEW JSON FIELDS
    temp.inv_supervisor_people_json = new PeopleJsonModel();
    temp.inv_supervisor_operation_json = new OperationJsonModel();
    temp.inv_supervisor_time_json = new TimeJsonModel();
      ...
      (data: InvSupervisorModel) => {
        ...
        alert(
          `Registro modificado satisfactoriamente, ${
            this.action.temp.inv_supervisor_id
          }, ${this.action.temp.inv_supervisor_name}`
        );
