...
export class InvAgentService {
  ...
  getSchedules(query?): Observable<any> {
    const accessToken = localStorage.getItem("accessToken");
    let filter;
    query = `{"where":{"inv_schedule_status":"A"}}`;
    if (query) {
      filter = `?filter=${query}&access_token=${accessToken}`;
    } else {
      filter = `?access_token=${accessToken}`;
  ...
  getCalendars(query?): Observable<any> {
    const accessToken = localStorage.getItem("accessToken");
    let filter;
    query = `{"where":{"inv_calendar_status":"A"}}`;
    if (query) {
      filter = `?filter=${query}&access_token=${accessToken}`;
    } else {
      filter = `?access_token=${accessToken}`;
    }
    const url_api = `${this.env.loopbackApiUrl}/api/InvCalendars?access_token=${accessToken}`;
    return this.http.get<any>(url_api, { headers: this.headers }).pipe(
      map(data => data),
      catchError(this.handleError)
    );
