  ...
  createRecord(query: AuxLineModel) {
    ...
    this.auxLineService.postRecord(query).subscribe(
      data => {
          ...
          alert(`Registro agregado satisfactoriamente, ${data.aux_line_id}, ${data.aux_line_name}`)
  ...
  editRecord(query: AuxLineModel) {
    ...
    this.auxLineService.putRecord(query).subscribe(
      data => {
        ...
        alert(`Registro modificado satisfactoriamente, ${this.action.temp.aux_line_id}, ${this.action.temp.aux_line_name}`)
...
sendTest() {
  ...
  this.action.temp = [{aux_line_id: 2, aux_line_name: "Supervisor 008", aux_line_status: "A"}];
