...
@Component({
  selector: 'app-crud-inv-assignation-detail',
  templateUrl: './crud-inv-assignation-detail.component.html',
  styleUrls: ['./crud-inv-assignation-detail.component.scss'],
  })
export class CrudInvAssignationDetailComponent implements OnInit {
  ...
  onFillForm() {
    ...
    this.registerForm = this.formBuilder.group({
      ...
      inv_break_chk: [this.selection.inv_break_chk, Validators.required],
  ...
  // CREATE RECORD
  createRecord(query: InvAssignationModel) {
    delete query.inv_break_class;
    this.invAssignationService.postRecord(query).subscribe(
      data => {
        alert(
  ...
  // CREATE RECORD
  editRecord(query: InvAssignationModel) {
    delete query.inv_break_class;
    this.invAssignationService.putRecord(query).subscribe(
      data => {
        this.selection = data;
  ...
  // DELETE RECORD
  deleteRecord(query: InvAssignationModel) {
    let id = this.action.temp.inv_break_id;

    this.invAssignationService.deleteRecord(id).subscribe(
      data => {
        this.selection = data;
        this.action.temp = this.selection;
  ...
  onChangeToAuxiliar() {
    this.selection = this.action.temp;
    this.selection.inv_break_productivity = 0;
    this.selection.inv_break_class = 'Auxiliar';
    this.onFillForm();
    this.show_submit_button = true;
