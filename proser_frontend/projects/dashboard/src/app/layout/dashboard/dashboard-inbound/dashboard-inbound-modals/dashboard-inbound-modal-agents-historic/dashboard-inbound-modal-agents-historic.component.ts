...
@Component({
  selector: 'app-dashboard-dashboard-inbound-modal-agents-historic',
  templateUrl: './dashboard-inbound-modal-agents-historic.component.html',
  styleUrls: ['./dashboard-inbound-modal-agents-historic.component.scss']
})
export class DashboardInboundModalAgentsHistoricComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dashboardInboundListAuditAgents(this.userSelection);
  }

  dashboardInboundListAuditAgents(userSelection: UserSelectionModel) {
    ...
    if (this.dashboardSelection) {
      this.dashboardInboundListsService
        .dashboardInboundListAuditAgents(this.dashboardSelection)
        .subscribe(
          res => {
            if (res) {
              this.rows = res;
